...
def train(model,
    ...
    for epoch in range(start_epoch + 1, end_epoch + 1):
        ...
        for i, data in enumerate(tqdm(trainloader)):
            ...
            onehot_labels = F.one_hot(labels, raw_logits.shape[1]).type(torch.FloatTensor).cuda()
            
            raw_loss = poly1_ce(criterion, raw_logits, onehot_labels)
            local_loss = poly1_ce(criterion, local_logits, onehot_labels)
            windowscls_loss = poly1_ce(criterion, proposalN_windows_logits,
                               onehot_labels.repeat(1, proposalN).reshape(-1, raw_logits.shape[1]))
        ...
        if eval_trainset:
            raw_loss_avg, windowscls_loss_avg, total_loss_avg, raw_accuracy, local_accuracy, overall_accuracy, local_loss_avg\
                = eval(model, trainloader, criterion, 'train', save_path, epoch)

            print(
                'Train set: raw accuracy: {:.2f}%, local accuracy: {:.2f}%, overall accuracy: {:.2f}%'.format(
                    100. * raw_accuracy, 100. * local_accuracy, 100. * overall_accuracy))
            ...
            with SummaryWriter(log_dir=os.path.join(save_path, 'log'), comment='train') as writer:
                ...
                writer.add_scalar('Train/overall_accuracy', local_accuracy, epoch)
        ...
        # eval testset
        raw_loss_avg, windowscls_loss_avg, total_loss_avg, raw_accuracy, local_accuracy, overall_accuracy,\
        local_loss_avg\
            = eval(model, testloader, criterion, 'test', save_path, epoch)

        print(
            'Test set: raw accuracy: {:.2f}%, local accuracy: {:.2f}%, overall accuracy: {:.2f}%'.format(
                100. * raw_accuracy, 100. * local_accuracy, 100. * overall_accuracy))
        ...
        with SummaryWriter(log_dir=os.path.join(save_path, 'log'), comment='test') as writer:
            ...
            writer.add_scalar('Test/overall_accuracy', local_accuracy, epoch)
