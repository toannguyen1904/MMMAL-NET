...
def eval(model, testloader, criterion, status, save_path, epoch):
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(testloader)):
            ...
            if set == 'CUB':
                # compute resized coordinates of boxes
    ...
    overall_accuracy = overall_correct / len(testloader.dataset)

    return raw_loss_avg, windowscls_loss_avg, total_loss_avg, raw_accuracy, local_accuracy, overall_accuracy, \
           local_loss_avg
