...
class APPM(nn.Module):
    ...
    def forward(self, proposalN, x, ratios, window_nums_sum, N_list, iou_threshs, DEVICE='cuda'):
        ...
        # feature map sum
        fm_sum = [torch.sum(avgs[i], dim=1) for i in range(len(ratios))] # batch_size x h x w
...
class MainNet(nn.Module):
    ...
    def forward(self, x, epoch, batch_idx, status='test', DEVICE='cuda'):
        ...
        #SCDA
        bottleneck_block = BottleneckBlock(in_channels=channel_size, fmap_size=(side_size, side_size), heads=4, out_channels=channel_size, pooling=False).cuda()
        fm = bottleneck_block(fm)
